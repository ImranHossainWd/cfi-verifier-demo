...
class OCRConfig:
    ...
    vision_model: Optional[str] = None     # passed to AnthropicVisionBackend if set
    vision_force: bool = False             # if True, escalate every page (testing)
    ...
    def _make_vision(self):
        ...
        if p == "anthropic":
            if self.config.vision_model:
                return AnthropicVisionBackend(model=self.config.vision_model)
